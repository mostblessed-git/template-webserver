pipeline {
    ...
    environment {
        ...
        def CONNECT = 'ssh -o StrictHostKeyChecking=no ubuntu@35.183.236.169'
    }
    stages {
        ...
        stage('Deploy') {
            steps {
                echo 'Deploying app'
                sshagent(['server-key']) {
                    sh 'scp -o StrictHostKeyChecking=no -i $SSH_CRED webapp.zip ubuntu@335.183.236.169:/home/ubuntu'
